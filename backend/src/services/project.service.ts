...
// Helper to verify a user owns the project
...
// Get DNS configuration instructions for custom domain setup
function getDomainConfigInstructions(domain: string): DomainConfigInstructions[] {
  ...
  return [
    ...
    },
    {
      type: 'A_RECORD',
      name: cleanDomain,
      value: '104.16.0.1', // Example Cloudflare IP
      ttl: 300,
...
export async function getDomainVerificationStatus(projectId: string, userId: string) {
  try {
    ...
    if (project.customDomain) {
      // Get real-time verification status for custom domains
      const verificationResult = await verifyCustomDomain(project.customDomain);
...
// Check domain availability for project creation
