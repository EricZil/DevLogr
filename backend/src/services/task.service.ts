...
// Helper to verify a user owns the project associated with a task/milestone
...
// Optimized milestone progress calculation with caching
export async function updateMilestoneProgress(milestoneId: string) {
  console.time(`updateMilestoneProgress-${milestoneId}`);
  
  // Optimized query: only fetch needed fields and use new indexes
  const tasks = await prisma.task.findMany({
    where: {
      milestoneId,
      status: { not: "CANCELLED" } // Filter out cancelled tasks in query
    },
    select: {
      id: true,
      status: true,
      subtasks: {
        select: { completed: true } // Only fetch completion status
      }
    }
  });
...
// Async wrapper for non-blocking progress updates
...
export async function getTasksForMilestone(milestoneId: string, userId: string) {
  ...
  // Use cache-or-fetch pattern for task retrieval
...
export async function updateTask(taskId: string, userId: string, taskData: any) {
  ...
  try {
    ...
    // STEP 4: Update task in database (measure DB query)
    ...
    // STEP 5: Invalidate caches (measure cache operations)
    await measureCacheOp('invalidateCache', existingTask.milestoneId, () =>
      invalidateCache.milestone(existingTask.milestoneId)
    );
    
    // STEP 6: CRITICAL PERFORMANCE FIX: Use async progress update to avoid blocking response
...
// ===================================
// Part 2: Comments and Time Entries
// ===================================
...
// ===================================
// Part 3: Stats and Project-Level
// ===================================

/**
 * Gets statistics for a single task.
 */
export async function getTaskStats(taskId: string, userId: string) {
    ...
    if (!fullTask) { // Should not happen due to verifyTaskOwnership, but for safety
...
/**
 * Gets all tasks for an entire project.
 */
