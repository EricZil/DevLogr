...
// Helper to verify a user owns the project associated with an update
...
// Validation schema for creating/updating an update
...
/**
 * Gets recent updates across all user projects.
 */
...
export async function updateAnUpdate(
  ...
  // Separate images from the rest of the data to handle serialization
  const { images, ...restOfData } = validatedData;

  const updatePayload: any = restOfData;

  // Only include the images field in the update if it was passed in the request
  if (images !== undefined) {
...
/**
 * Gets update statistics for a project.
 */
