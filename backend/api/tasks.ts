...
export default async function handler(
  ...
  try {
    ...
    if (action === 'stats') {
      ...
      const stats = await getTaskStats(id, userId);
      return res.status(200).json(stats);
    }

    // Handle task time logging
    if (action === 'time') {
      if (req.method !== "POST") {
        res.setHeader("Allow", ["POST"]);
        return res.status(405).json({ error: "Method not allowed" });
      ...
      const newTimeEntry = await logTimeForTask(id, userId, req.body);
      return res.status(201).json(newTimeEntry);
    }

    // Handle task subtasks
    if (action === 'subtasks') {
      if (req.method !== "POST") {
        res.setHeader("Allow", ["POST"]);
        return res.status(405).json({ error: "Method not allowed" });
