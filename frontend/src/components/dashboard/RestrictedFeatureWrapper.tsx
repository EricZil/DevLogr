...
export default function RestrictedFeatureWrapper({
  ...
  const { status, loading, canAccessFeatures } = useDomainVerification({
    projectId,
    autoRefresh: true,
    refreshInterval: 30000 // Check every 30 seconds for unverified domains
  });
  ...
  // If no custom domain or domain is verified, allow full access
  if (canAccessFeatures) {
    return <div className={className}>{children}</div>;
  }

  // Show restriction message for unverified custom domains
  return (
    <div className={`${className}`}>
