...
export default function ProjectSetup({ isOpen, onClose, onComplete }: ProjectSetupProps) {
  ...
  const validateProjectName = (name: string): ValidationError[] => {
    ...
    // Warning for names that might be problematic
  ...
  const validateUrl = (url: string, type: 'github' | 'website' | 'twitter'): ValidationError[] => {
    ...
    if (!url.trim()) return errors; // URLs are optional
    ...
    try {
      ...
      switch (type) {
        ...
        case 'twitter':
          ...
          break;
        case 'website':
          // More lenient for website URLs, just check for suspicious TLDs
  ...
  // ===========================
  // EXISTING FUNCTIONS (keeping the same)
  // ===========================
  ...
  const debouncedCheckDomain = useCallback((domain: string) => {
    ...
    // Reset verification state while user is typing
  ...
  const canProceed = () => {
    switch (step) {
      case 1:
        // Must have valid name and description (no errors)
        return !hasErrors(['name', 'description']) && projectData.name.trim().length > 0;
      case 2:
        return true; // Domain type selection
      case 3:
        if (projectData.domainType === 'subdomain') {
          // Must have valid slug and it must be available
          return !hasErrors(['slug']) && 
                 projectData.slug.length >= 3 && 
                 slugAvailable === true;
        } else {
          // Must have valid custom domain and it must be available
      ...
      case 4:
        // Check validation for optional fields that were filled
      ...
      case 5:
        // Final validation - all fields must be valid
