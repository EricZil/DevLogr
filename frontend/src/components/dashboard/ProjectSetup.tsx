...
export default function ProjectSetup({ isOpen, onClose, onComplete }: ProjectSetupProps) {
  ...
  const debouncedCheckSubdomain = useCallback((subdomain: string) => {
    ...
    // Only check if subdomain is valid length
    if (!subdomain || subdomain.length < 3) return;
  ...
  const createProject = async () => {
    ...
    try {
      const response = await api.createProject({
        ...
        // The service does not handle these yet, but we can pass them
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 2:
        return true; // Local development setup, no validation needed
      case 3:
        return projectData.subdomain.length >= 3 && subdomainAvailable === true;
      case 4:
        return true; // Tech stack and features are optional
      case 5:
