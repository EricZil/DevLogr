...
function TaskCard({ task, isDragging }: { task: Task; isDragging?: boolean }) {
  // Always call useSortable hook (hooks must be called unconditionally)
  ...
  return (
    ...
    >
      <div className="space-y-1.5">
        ...
        {/* Display milestone tag if available */}
        {task.milestone && (
...
export default function EnterpriseKanban({ tasks, onTaskMove, onTaskClick }: EnterpriseKanbanProps) {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    if (foundTask) {
      ...
      // Calculate initial card position and drag offset
      if (event.activatorEvent instanceof MouseEvent) {
        const mouseEvent = event.activatorEvent;
        
        // Find the element being dragged using DOM
        const element = document.getElementById(active.id.toString());
        if (element) {
          // Store the original element for cloning
          ...
          // Calculate offset between mouse position and card top-left corner
          setDragOffset({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Determine target column - prioritize dragTarget over other detection methods
    let targetColumn: Task['status'] | null = null;
    
    // First check if dragTarget is set (our enhanced detection)
    if (dragTarget && STATUS_COLUMNS.some(col => col.id === dragTarget)) {
      targetColumn = dragTarget as Task['status'];
    } 
    // Then check if over property is set (fallback)
    else if (over) {
      // Check if over is a column
      if (STATUS_COLUMNS.some(col => col.id === over.id)) {
        targetColumn = over.id as Task['status'];
      } 
      // Check if over is a task and get its column
      else {
    ...
    // If we found a target column and it's different from the current status, update the task
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      >
        ...
        {/* Replace DragOverlay with our custom implementation */}
