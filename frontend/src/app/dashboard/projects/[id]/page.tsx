...
export default function ProjectManagement() {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/projects/${projectId}/basic`, {
          headers: {
  ...
  const updateBasicInfo = async (data: Record<string, unknown>) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/projects/${projectId}/basic-info`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  // Load tags when settings tab is active
  useEffect(() => {
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-8 py-12">
        ...
        {activeTab === 'settings' && (
          <div className="max-w-6xl space-y-8">
            ...
            {/* TODO: Banner & Media Section - Future Implementation
                Features to implement:
                - Project banner upload area with drag & drop interface
                - Banner preview with remove option  
                - 16:9 aspect ratio recommendation
                - Media gallery management
                - File upload handling for banners/media
                - Image optimization and resizing
            */}

            {/* TODO: Export & Backup Section - Future Implementation  
                Features to implement:
                - Export project as JSON (complete project data)
                - Generate PDF report (summary with stats and timeline)
                - Create project backup (full data backup)
                - Data portability features
                - Import/export settings and preferences
                - Scheduled backup options
            */}

            {/* TODO: Danger Zone Section - Future Implementation
                Features to implement:
                - Archive project (reversible, hidden from public view)
                - Transfer ownership to another user (irreversible)
